...
for i in 0 1 2; do
  ...
  sed -i "s/initial_token\:\ replaceme/initial_token\:\ ${tokens[$(($i+1))]}/g" $workspace/conf-$i/cassandra.yaml
  sed -i "s/listen_address\:\ replaceme/listen_address\:\ 127.0.0.$(($i+1))/g" $workspace/conf-$i/cassandra.yaml
  sed -i "s/rpc_address\:\ replaceme/rpc_address\:\ 127.0.0.$(($i+1))/g" $workspace/conf-$i/cassandra.yaml
  ...
  JVM_EXTRA_OPTS="$CASSANDRA_OPTS -DflakyAgent.pid=$i"

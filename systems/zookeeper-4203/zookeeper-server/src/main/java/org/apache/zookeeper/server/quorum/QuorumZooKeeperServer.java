...
public abstract class QuorumZooKeeperServer extends ZooKeeperServer {
    ...
    private final Object syncStateObject = new Object();
    public static volatile boolean leaderStateTestFlag = true;
    @Override
    protected void setState(State state) {
        synchronized (syncStateObject) {
            // this.state and state
            // de1 state = x, this.state = y => does not satisfy this condition
            // de2 state = x, this.state = y => we don't need to try de2
            // de2 state = x', this.state = y' => we should try de2
            if (state == State.RUNNING) {
                if (this.state == State.ERROR) {
                    leaderStateTestFlag = false;
                }
            }
            this.state = state;
        }

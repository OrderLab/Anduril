...
public class MirrorConnectorsIntegrationTest {
    ...
    private static final int NUM_RECORDS_PER_PARTITION = 1;
    ...
    public void testReplicationWithEmptyPartition() throws Exception {
        ...
        int expectedRecords = NUM_RECORDS_PER_PARTITION * NUM_PARTITIONS;
        try {
            produceMessages1(primary, topic, "message-1-", NUM_PARTITIONS);
        } catch (KafkaException e) {
            log.info("Exception while producing");
            expectedRecords = NUM_RECORDS_PER_PARTITION * (NUM_PARTITIONS - 1);
        }
    ...
    private void produceMessages1(EmbeddedConnectCluster cluster, String topicName, String msgPrefix) {
        produceMessages1(cluster, topicName, msgPrefix, NUM_PARTITIONS);
    }

    private void produceMessages1(EmbeddedConnectCluster cluster, String topicName, String msgPrefix, int numPartitions) {
        // produce the configured number of records to all specified partitions
        int cnt = 0;
        for (int p = 0; p < numPartitions; p++) {
            for (int r = 0; r < NUM_RECORDS_PER_PARTITION; r++) {
                //if (p == (numPartitions - 1)) {
                //    throw new KafkaException("Inject Error");
                //}
                cluster.kafka().produce(topicName, p, "key", msgPrefix + cnt++);
            }
        }

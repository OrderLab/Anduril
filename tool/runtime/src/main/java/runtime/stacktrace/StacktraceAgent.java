...
public final class StacktraceAgent {
    private static final Logger LOG = LoggerFactory.getLogger(runtime.stacktrace.StacktraceAgent.class);

    private static final AtomicReference<StacktraceRemote> stub = new AtomicReference<>(null);
    ...
    public static final String RMI_NAME = "rmi_flaky_stacktrace";
    ...
    public static void inject(final int id, final String exceptionName) throws Throwable {
        if (disableAgent) {
            return;
        }
        ...
        if (distributedMode) {
            if (getStub().inject(id, pid) == 1) {
                throw exception;
            }
        } else {
            if (injectionManager.inject(id, pid)) {
                throw exception;
            }
        }
    }
    ...
    static private StacktraceRemote getStub() {
        return stub.updateAndGet(s -> {
            ...
            try {
                ...
                return (StacktraceRemote) registry.lookup(RMI_NAME);
    ...
    static public runtime.stacktrace.InjectionManager injectionManager = null;
    ...
    static public void main(final String[] args) throws Throwable {
        ...
        injectionManager = new runtime.stacktrace.InjectionManager(args[0], args[1], args[2]);
        if (distributedMode) {
            ...
            final StacktraceStub s = new StacktraceStub();

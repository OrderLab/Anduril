...
fs.readFileSync(bad_run_log).toString().split('\n').forEach(line => {
  if (line.startsWith('Time: ') && ending == 0) {
	...
	if (ending == 1 && line.startsWith('1) testSourceTaskNotBlockedOnShutdownWithNonExistentTopic(org.apache.kafka.connect.integration.ConnectWorkerIntegrationTest)')) {
	  ending = 2
  }
	if (ending == 2 && line.startsWith('java.lang.AssertionError: Connector and all tasks were not stopped in time')) {
    ending = 3
  }
	if (ending == 3 && line.includes('at org.apache.kafka.connect.integration.ConnectWorkerIntegrationTest.testSourceTaskNotBlockedOnShutdownWithNonExistentTopic(ConnectWorkerIntegrationTest.java:325)')) {
	  symptom = true
  }
})
if (!symptom) {

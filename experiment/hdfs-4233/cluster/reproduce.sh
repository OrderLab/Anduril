...
INJECT_HADOOP_OPTS="-Dflaky.distributedMode=true -Dflaky.disableAgent=true"
HADOOP_OPTS="-Dflaky.distributedMode=true -Dflaky.disableAgent=true"

$workspace/setup.sh
cp -r $workspace/init-store/current $workspace/store-1/

HADOOP_OPTS="$INJECT_HADOOP_OPTS" $workspace/start-cluster.sh

for i in 0 1 2 3; do
  pid=`cat $workspace/logs-$i/*.pid`
  if [ $(ps -p $pid | wc -l) -eq 1 ]; then
    echo "some node fails to start"
    HADOOP_OPTS="$HADOOP_OPTS" $workspace/stop-cluster.sh
    exit 0
  fi
done
...
for i in 1 2 3 4; do
  ...
  HADOOP_OPTS="$HADOOP_OPTS" $workspace/client.sh -mkdir /$i &
  pid=$!
  for j in {0..5}; do
    sleep 1
    if [ $(ps -p $pid | wc -l) -eq 1 ]; then

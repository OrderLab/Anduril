...
fs.readFileSync(bad_run_log).toString().split('\n').forEach(line => {
  if (line.startsWith('Time: ') && ending == 0) {
	...
	if (ending == 1 && line.startsWith('1) testDatalossWhenInputError(org.apache.hadoop.hbase.regionserver.wal.TestWALReplay)')) {
	  ending = 2
  }
	if (ending == 2 && line.startsWith('java.lang.AssertionError: Data Loss!')) {
    ending = 3
  }
	if (ending == 3 && line.includes('at org.apache.hadoop.hbase.regionserver.wal.TestWALReplay.testDatalossWhenInputError(TestWALReplay.java:1030)')) {
	  symptom = true
  }
})
if (!symptom) {

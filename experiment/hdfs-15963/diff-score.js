...
fs.readFileSync(bad_run_log).toString().split('\n').forEach(line => {
  if (line.startsWith('Time: ') && ending == 0) {
	...
	if (ending == 1 && line.startsWith('1) testReleaseVolumeRefIfExceptionThrown(org.apache.hadoop.hdfs.TestDataTransferProtocol)')) {
	  ending = 2
  }
	if (ending == 2 && line.startsWith('java.lang.AssertionError: They should be equal')) {
    ending = 3
  }
	if (ending == 3 && line.includes('at org.apache.hadoop.hdfs.TestDataTransferProtocol.testReleaseVolumeRefIfExceptionThrown(TestDataTransferProtocol.java:616)')) {
	  symptom = true
  }
})
if (!symptom) {
